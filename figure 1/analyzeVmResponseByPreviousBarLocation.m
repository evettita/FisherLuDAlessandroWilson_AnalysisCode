...
% INPUT wanted trials herejust told
possibleTrials = [1:5];
...
for fileNum = 1 : length ( trialFilesList )
    ...
    for j = ( minXPos : maxXPos)  % loop over all possible X-pos values and pull out traces for each that has data
        ...
        if( ~ isempty(epochStartInds) )
            ...
            % 
            currentBarPosition(j, 1:length( currBarPositionShown ) ) = currBarPositionShown;
    ...
    processedData(fileNum).currentBarPosition = currentBarPosition;
...
%% save current plot
dir = '/Users/evettita/Dropbox (HMS)/FisherLuWilson ms/revision/Figures/Figure 1/';
fileName = [ dir 'fly288_-97DegREsponse.eps' ];
print( fileName, '-dpdf');
